...
class UnoGameCardDeck(DirectFrame):
	...
	def updateCardToFollowGui(self):
		...
		if self.unoGame.cardToFollow[-2:] == str(UGG.CARD_BLUE):
			self.cardToFollowGui = OnscreenImage(image = gui.find('**/mg_uno_numcards_blue_blank'),
				parent = base.a2dRightCenter)
		elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_RED):
			self.cardToFollowGui = OnscreenImage(image = gui.find('**/mg_uno_numcards_red_blank'),
				parent = base.a2dRightCenter)
		elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_GREEN):
			self.cardToFollowGui = OnscreenImage(image = gui.find('**/mg_uno_numcards_green_blank'),
				parent = base.a2dRightCenter)
		elif self.unoGame.cardToFollow[-2:] == str(UGG.CARD_YELLOW):
			self.cardToFollowGui = OnscreenImage(image = gui.find('**/mg_uno_numcards_yellow_blank'),
				parent = base.a2dRightCenter)
