...
class MotionTrails(DirectObject):
    ...
    def takeSnapShot(self, task):
        if (task.time > self.nextclick):
            ...
            #img = OnscreenImage(image = shot, parent = render2d, color = (1.0,1.0,1.0,0.99))
            #taskMgr.doMethodLater(0.5, self.delSnapShot, "snapshotdel", extraArgs = [shot, img], appendTask = True)
        return Task.cont
        
    def delSnapShot(self, shot, img, task):
        img.destroy()
        shot.clear()
